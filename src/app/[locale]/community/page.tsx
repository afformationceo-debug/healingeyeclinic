...
export async function generateMetadata(props: { params: Promise<{ locale: string }> }): Promise<Metadata> {
    ...
    const seo = getSeoForPage('community', locale);
    const messages = await getMessages({ locale }) as { Metadata: { community: { title: string; description: string } } };

    const title = seo?.title_tag || messages.Metadata.community.title;
    const description = seo?.meta_description || messages.Metadata.community.description;
