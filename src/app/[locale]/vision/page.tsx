...
export async function generateMetadata(props: { params: Promise<{ locale: string }> }): Promise<Metadata> {
    ...
    const seo = getSeoForPage('vision', locale);
    const messages = await getMessages({ locale }) as { Metadata: { vision: { title: string; description: string } } };

    const title = seo?.title_tag || messages.Metadata.vision.title;
    const description = seo?.meta_description || messages.Metadata.vision.description;
