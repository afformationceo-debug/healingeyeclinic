...
export async function generateMetadata(props: { params: Promise<{ locale: string }> }): Promise<Metadata> {
    ...
    const seo = getSeoForPage('cataract', locale);
    const messages = await getMessages({ locale }) as { Metadata: { cataract: { title: string; description: string } } };

    const title = seo?.title_tag || messages.Metadata.cataract.title;
    const description = seo?.meta_description || messages.Metadata.cataract.description;
